<template>
    <header>
        <div class="cont header__container">
            <div class="logo">
                <NuxtLink to="/" class="logo-text">Платформа TRI</NuxtLink>
                <NuxtLink to="/games" class="nav-link">Игры</NuxtLink>
                <NuxtLink to="/profile" class="nav-link">Профиль</NuxtLink>
            </div>
            <div class="menu">
                <template v-if="userStore.isAuthenticated">
                    <span class="menu__hello">{{ userStore.me.username }}</span>
...
.nav-link:hover { color: #0051a8; }
.nav-link.router-link-active { color: #0051a8; font-weight: 600; }

.menu { display: flex; gap: 1.2rem; align-items: center; }
.menu__hello { font-size: 1.4rem; color: #475569; }
