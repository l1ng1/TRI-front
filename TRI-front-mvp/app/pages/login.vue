<template>
    <form class="login" @submit.prevent="onSubmit">
        ...
        <div class="login__inputs-box">
            ...
            <label class="login__label">
                Пароль
                <input v-model="password" type="password" class="login__input" autocomplete="current-password" required>
            </label>
        </div>
        ...
        <button class="btn login__btn" :disabled="userStore.authLoading" type="submit">
            {{ userStore.authLoading ? 'Входим...' : 'Войти' }}
        </button>
        <p class="login__text">
            Еще нет аккаунта?
            <NuxtLink to="/registration" class="login__link">Зарегистрироваться</NuxtLink>
        </p>
    </form>
</template>
